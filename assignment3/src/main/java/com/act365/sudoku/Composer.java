 ...
 * MostCandidates strategy. It runs as a thread because composition is a lengthy
 * process that the caller might well choose to terminate.
 */
public class Composer extends Thread
{

    private List<Grid> puzzles;  // jcc - changed from Vector to interface List<Grid>, and added private to ensure thread safety
    ...
    final static String[] featuredGrades =
    {
        "Easy",
        "Medium",
        "Hard",
    ...
    /**
     * Generates a new thread that will attempt to construct a Su Doku puzzle
     * with 'maskSize' cells on initial display. The puzzle will be copied into
     * 'grid'. When 'randomize' is selected, the mask will be selected randomly
     * and the RadomMostCandidates strategy will be used to complete it.
     *
     * @param gridContainer object to be notified when a puzzle has been
     * composed
     * @param boxesAcross
     ...
     * @param composeSolverThreshold tree depth beyond which the compose solver
     * will be invoked
            ...
            boolean xmlFormat) throws Exception
    {
        ...
        puzzles = Collections.synchronizedList(new ArrayList<>());  // jcc - changed from Vector to synchronized ArrayList
    ...
    /**
     * Called by a Solver object in order to indicate that a solution has been
     * found.
     *
     * @see Solver
    ...
    public synchronized void addSolution(int solverIndex)
    {
        ...
        if (!puzzles.contains(puzzle))
        {
            ...
            lch.reset();
            puzzles.add(puzzle);  // jcc - changed from addElement to add
            if (output != null)
            {
                if (xmlFormat)
                {
    ...
    /**
     * Called by a Solver object in order to indicate that all possible
     * solutions to the puzzle have been considered.
     *
     * @see Solver
    ...
    /**
     * Starts a new Solver thread to find puzzles for a fixed initial mask.
     *
     * @see Solver
    ...
    /**
     * Starts a Composer thread. The thread will start a number of Solvers and
    ...
    public void run()
    {
        ...
        synchronized (this)
        {
            ...
            if (gridContainer != null)
            {
                if (puzzles.size() > 0)
                {
                    gridContainer.setGrid(puzzles.get(0));  // jcc - replace elementAt with get and removed cast to Grid
                }
            } else
            {
                ...
                } else
                {
                    ...
                    if (nSolns > 0)
                    {
                        ...
                        System.out.println((puzzles.get(mostComplex)).toString());   // jcc - replace elementAt with get and removed cast to Grid
                    }
                }
            }
        }
     ...
     * <br><code>[-a across] [-d down]</code> define the dimensions of the
     * puzzles to be composed. The default values are three in each dimension.
     * <br><code>[-ms max solns|-mm max masks]</code> defines optional
     * termination conditions. The app will exit if <code>max solns</code>
     * puzzles have been generated or <code>max masks</code> masks have been
     * considered, whichever occurs sooner.
     * <br><code>[-mu max unwinds]</code> stipulates a limit on the number of
     * unwinds permitted on a single mask.
     * <br><code>[-mu max unwinds]</code> stipulates a limit on the complexity
     * permitted on a single mask.
     * <br><code>[-s solvers]</code> stipulates the number of solver (or,
     * equivalently, threads) to execute simultaneously. The default is 3.
     * <br><code>[-c threshold]</code> stipulates the tree depth beyond which
     * the compose solver will be invoked. The default value is 0.
     * <br><code>[-v]</code> stipualtes whether the Composer should run in
     * verbose mode. The default is no.
     * <br><code>[-f]</code> stipulates that the full set of Least Candidates
     * Hybrid algorithms should be used to solve.
     * <br><code>[-shuffle]</code> stipulates that the puzzles should be
     * randomly shuffled.
     * <br><code>[-xml]</code> stipulates that the output should be in XML
     * format.
     * <br><code>-i</code> stipulates that the initial mask should be read from
     * standard input.
     * <br><code>#cells</code> stipulates the number of initially-filled cells
     * to appear in the puzzles.
     */
    public static void main(String[] args)
    {
        ...
        while (i < args.length - 1)
        {
            ...
            if (sign != 0)
            {
                ...
                {
                    if (strategy.equalsIgnoreCase("ds"))
                    {
                        disjointSubsetsFilter = sign;
                    } else
                    {
                        if (strategy.equalsIgnoreCase("2ds"))
                        {
                            twoSectorDisjointSubsetsFilter = sign;
                        } else
                        {
                            if (strategy.equalsIgnoreCase("svc"))
                            {
                                singleValuedChainsFilter = sign;
                            } else
                            {
                                if (strategy.equalsIgnoreCase("mvc"))
                                {
                                    manyValuedChainsFilter = sign;
                                } else
                                {
                                    if (strategy.equalsIgnoreCase("nishio"))
                                    {
                                        nishioFilter = sign;
                                    } else
                                    {
                                        if (strategy.equalsIgnoreCase("guess"))
                                        {
                                            guessFilter = sign;
                                        } else
                                        {
